#!/usr/bin/env python3
"""Import Lee et al. (2020) fifth-force constraint curve.

This importer validates and ingests a manually-digitized CSV file from Lee et al. (2020).
The CSV must be placed in data/raw/fifth_force/mm_cm_constraints/ before running.

Source: Lee et al., "Test of the Gravitational Inverse-Square Law at Millimeter Ranges",
Physical Review Letters 124, 101101 (2020)
arXiv: 2002.11761
"""
...
# Expected source_id
SOURCE_ID = "lee_arxiv2020_digitized"

# Expected file location
EXPECTED_INPUT = project_root / "data" / "raw" / "fifth_force" / "mm_cm_constraints" / "lee_arxiv2020_digitized_contract.csv"

# Expected coverage (approximate, for validation)
EXPECTED_LAMBDA_MIN = 5e-5  # ~52 μm
EXPECTED_LAMBDA_MAX = 3e-3  # ~3.0 mm
...
def validate_csv(csv_path: Path) -> pd.DataFrame:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            ...
            f"Please digitize Lee 2020 Figure 5 (bottom panel) and place the CSV at:\n"
            f"  {csv_path}\n"
...
def main():
    ap = argparse.ArgumentParser(
        description="Import Lee et al. (2020) fifth-force constraint curve"
    )
    ap.add_argument(
        "--input-csv",
        type=Path,
    ...
    contract_csv = project_root / "data" / "raw" / "fifth_force" / "lee_arxiv2020_digitized_contract.csv"
    ...
    print(f"\n✅ Successfully imported Lee et al. (2020) constraint")
    print(f"   Contract CSV: {contract_csv}")
    print(f"   Validated CSV: data/processed/lee_arxiv2020_digitized_contract_validated.csv")
    print(f"   Provenance: results/fifth_force/lee_arxiv2020_digitized_contract_provenance.json")
