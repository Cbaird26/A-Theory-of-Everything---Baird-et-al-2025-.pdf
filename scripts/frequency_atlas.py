...
- This script maintains the original conversion function names for backward compatibility
...
# Backward compatibility: export constants with original names
C_LIGHT = C
H_PLANCK = H
E_CHARGE_LEGACY = E_CHARGE
HBAR = H / (2 * math.pi)

# Particle masses (CODATA 2018)
M_ELECTRON = 9.109_383_7015e-31  # Electron mass (kg)
M_PROTON = 1.672_621_92369e-27  # Proton mass (kg)
...
def timescale_to_freq(T_seconds: float) -> float:
...
def energy_to_freq(E_joules: float) -> float:
...
def mass_to_freq(m_kg: float) -> float:
...
def length_to_freq_wave(lam_meters: float) -> float:
...
def length_to_freq_yukawa(lam_meters: float) -> float:
...
def eV_to_freq(E_eV: float) -> float:
...
def GeV_to_freq(E_GeV: float) -> float:
...
def build_entries() -> List[Entry]:
    ...
    entries.append(Entry(label="Electron rest energy (Compton frequency)", f_hz=f_from_mass_kg(M_ELECTRON)))
    entries.append(Entry(label="Proton rest energy (Compton frequency)", f_hz=f_from_mass_kg(M_PROTON)))
    ...
    entries.append(Entry(label="Planck frequency (1 / Planck time)", f_hz=PLANCK_FREQUENCY))
...
def print_frequency_table():
    """Print frequency ladder table (backward compatibility)."""
    entries = build_entries()
    print(to_markdown(entries))


def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Generate frequency atlas (markdown or CSV)")
...
if __name__ == "__main__":
    # Backward compatibility: if run without args, print table
    if len(sys.argv) == 1:
